...
One line per entity, grouped into five families. The full treatment of each is in
`01-data-model.md`; this list is the map. Every line comes from the schemas under
`schema/`, the records they govern, and the tools that derive from them.
...
| 8 | Infrastructure Shape | One kind of estate that scenarios are mapped onto: the AI stack, a web application behind a gateway, an endpoint population. Family, where it runs, who owns it, its defining gap. The AI stack is the catalog's shape; the others are proposed reference data. |
| 8a | Shape Layer | One layer of one shape: L0 Infrastructure through L4 Application for the AI stack, W0 Edge through W4 Control plane for a web shape. Code, name, what sits here, components, why it matters. Where an attack achieves its objective. |
| 8b | Seam Tag | The vocabulary, per shape, of where a single attack move operates, such as Host / net, each mapped to the layer it lands on. A step may carry a tag outside the list; the validator flags it for review. |
| 8c | Emitted Source Category | The categories of system an estate of a shape emits evidence from, such as EDR / endpoint or AI gateway. The vocabulary behind an evidence row's emitted-at field. Categories, not products. |
...
| 10 | Step Seam Tag (on the step) | The tag each attack step carries, drawn from the shape's seam vocabulary. Capped at 18 characters. Free text is allowed on a draft and flagged on a published record. |
...
The records a person stands behind.

| # | Entity | One line |
|---|---|---|
| 13 | Scenario | The unit of the library. One attack or failure story with a title, a plain language summary, a classification including the infrastructure shape it is mapped onto, a priority with reasons, and a status that moves draft, in review, published, retired. |
...
| 48 | Snapshot | The immutable record of one metrics run for one organization and one infrastructure shape: the full framework version tuple with artifact hashes, the library state, the three metric families, and the ledgers of what was added, retired, or rescored. Kept forever. A correction is a new snapshot that says which one it supersedes. |
| 49 | Rollup Metrics | Coverage, exposure, and maturity for one shape of the library at one moment, each with its completeness companion. Reported side by side, never combined, and never blended across shapes. An unscored row is absent, not zero. |
...
Not stored as state, but each needs an endpoint that accepts it, so they are named here.
...
## Not entities, but worth naming so nobody models them
...
## Why there are more entities than the schemas suggest

Thirteen schemas describe the records, but several records hold more than one entity. The infrastructure shape record holds the shape, its layers, its seams and its emitted categories. The
evidence row splits into a shared row and a per organization assessment with its own
header. The environment brings its targets and components. Discovery mode brings a run and
an observation. The framework index brings tactics, mitigations, and pinned artifacts. And
three computed views, score provenance, rollup metrics, and readiness, were being produced
without a name. The two interface documents are named so the import and session endpoints
are in scope.
